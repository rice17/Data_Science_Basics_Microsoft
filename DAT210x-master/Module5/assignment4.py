...
#
# .. your code here ..
data = pd.read_csv('/home/abhishar/Desktop/DatS/DAT210x-master/Module5/Datasets/Wholesale customers data.csv')
data = data.fillna(0)
#
...
#
# .. your code here ..
data = data.drop(axis = 1, labels = ['Channel', 'Region'])
...
# .describe() method, or even by using the built-in pandas `.plot.hist()
#
# .. your code here ..
data.plot.hist()
data.describe()
...
for col in data.columns:
  # Bottom 5
  sort = data.sort_values(by=col, ascending=True)
  if len(sort) > 5: sort=sort[:5]
  for index in sort.index: drop[index] = True # Just store the index once

  # Top 5
  sort = data.sort_values(by=col, ascending=False)
  if len(sort) > 5: sort=sort[:5]
  for index in sort.index: drop[index] = True # Just store the index once
...
data.drop(inplace=True, labels=drop.keys(), axis=0)
print data.describe()
...
#T = preprocessing.StandardScaler().fit_transform(data)
#T = preprocessing.MinMaxScaler().fit_transform(data)
#T = preprocessing.MaxAbsScaler().fit_transform(data)
...
if PLOT_TYPE_TEXT:
  ...
  for i in range(len(T)): ax.text(T[i,0], T[i,1], data.index[i], color=c[labels[i]], alpha=0.75, zorder=600000)
...
if PLOT_VECTORS: drawVectors(T, display_pca.components_, data.columns, plt)
