...
zero = []
import os
os.chdir('/home/abhishar/Desktop/DatS/DAT210x-master/free-spoken-digit-dataset-master/recordings')
for i in os.listdir('.'):
	if i[0] == '0':
...
#
# .. your code here ..
data = pd.DataFrame(data = zero, dtype = np.int16)
...
random_idx = rng.randint(data.shape[0])
test  = data[random_idx]
train = np.delete(data, [random_idx], axis=0)
...
#
# .. your code here ..
X_test = test.iloc[:, range(0, int(Provided_Portion*n_audio_samples))]
y_test = test.iloc[:, range(int(Provided_Portion*n_audio_samples)), int(n_audio_samples)]
#
...
#
# .. your code here ..
X_train = train.iloc[:, range(0, Provided_Portion*n_audio_samples)]
y_train = train.iloc[:, range(Provided_Portion*n_audio_samples), n_audio_samples]
#
